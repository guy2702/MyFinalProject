...
/*
 * 1. שליפה: האפליקציה מזהה את ה-UID של המשתמש ומאזינה לנתונים שלו ב-Firebase.
 * 2. זמן אמת (Realtime): בזכות ValueEventListener, כל שינוי בענן מעדכן את המסך אוטומטית.
 * 3. אדפטר (Adapter): הנתונים עוברים לאדפטר, שמחשב ומציג את מספר השייק וכמות המרכיבים שבו.
 * 4. לחיצה: השייק נשמר זמנית, ועוברים למסך הפרטים עם הדגל (isAdminView = false) כדי להסתיר הרשאות ניהול.
 */
