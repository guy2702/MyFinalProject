...
/**
 * --- סיכום מחלקת AddItem (מסך הוספת פריט למנהל) ---
 * * תפקיד המסך:
 * מסך ניהול המאפשר למנהל המערכת (Admin) להוסיף חומרי גלם (פריטים) חדשים למאגר הנתונים
 * של האפליקציה (Firebase). הפריטים שנוספים כאן יוצגו לאחר מכן ללקוחות במסכי הרכבת השייק.
 * * * זרימת הנתונים והלוגיקה (Data Flow & Logic):
 * 1. קלט משתמש (UI): המסך אוסף נתונים תזונתיים מתיבות טקסט (EditText), קטגוריה מתוך
 * תפריט נפתח (Spinner), ומטרה (מסה/חיטוב) מתוך כפתורי רדיו (RadioGroup).
 * 2. טיפול בתמונה (Media): המסך מאפשר למנהל לפתוח את המצלמה או את גלריית המכשיר
 * (באמצעות Intent). לאחר בחירת התמונה (ב-onActivityResult), היא מומרת למחרוזת
 * טקסט מסוג Base64 בעזרת מחלקת ImageUtil, כדי שיהיה ניתן לשמור אותה במסד הנתונים.
 * 3. יצירת המודל (Data Binding): הקוד בודק וממיר את כל הקלטים למספרים, מחולל מזהה
 * ייחודי (ID) בעזרת ה-DatabaseService, ובונה אובייקט חדש מסוג Item.
 * 4. שמירה בענן: הפריט נשלח ל-Firebase. הפעולה מתבצעת באופן אסינכרוני (עם Callback).
 * בסיום (הצלחה או שגיאה), המערכת משתמשת ב-runOnUiThread כדי להציג הודעת Toast
 * בטוחה למשתמש ולסגור את המסך (finish).
 */
